...
2. Clone your fork.
3. Install [Task](https://taskfile.dev/): `brew install go-task` (macOS) or `sudo snap install task --classic` (Linux).
4. `task setup` — enables GitHub pages for your fork (needs the `gh` CLI, otherwise it prints the manual steps).
5. `task serve` — opens the presentation garage locally at <http://localhost:8000>.
6. `task open` — opens the presentation garage published on GitHub pages.

> Advanced: GitHub pages can publish any branch you like, the default is `main`.
...
| ------------------------ | ------------------------------------------------------------------------------------------------------- |
| `task setup -- <branch>` | Enable GitHub pages for your fork (no branch = `main`)                                                  |
...
`task bundle` inlines every asset of a reveal.js presentation into one HTML file under `dist/`, installing
`monolith` first if it is missing.
PDF presentations are skipped because the PDF is already self-contained.
...
- One folder per presentation under `presentations/`, containing an `index.html` w.
